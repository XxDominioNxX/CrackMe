...
###### MainActivity: 
1. verifyLibs() – проверка целостности загружаемых библиотек и classes.dex
2. onCreate() – isDebuggerConnected, RootDetection
...
-	При провале любой проверки выполняется **MainActivity -> ShowDialog** и приложение закрывается
-	Патч .apk
...
- Переменные переименованы для удобства. 
- Видна проверка по длине 24
...
1d0811130f1749150d0003195a1d1315080e5a0017081314 (24 байта)

**Скрипт для получениия флага:**
``` csharp
...
- В libfoo.so есть функция, выполняющаяся в фоновом потоке и проверяющая память процесса на использование frida и xposed. Если проверка не проходит, приложение закрывается.
- Патчим в Ida строки "frida" и "xposed" на другие, чтобы обойти проверки
...
    while ( !strstr((const char *)&v3, "frida") && !strstr((const char *)&v3, "xposed") );
